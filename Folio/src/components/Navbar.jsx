  ...
  return (
    <header className="flex flex-col md:flex-row justify-between items-start md:items-center p-6 shadow-sm bg-white dark:bg-gray-800 dark:text-white">
      <div className="flex items-center space-x-4 w-full md:w-auto">
        ...
        <div
          className="relative w-full md:w-96 mt-4 md:mt-0"
            ...
            <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg z-10 max-h-72 overflow-y-auto p-4">
              ...
              <ul className="mb-4">
                {topGainers.map((stock, idx) => (
                  <li
                    key={idx}
                    className="flex items-center gap-2 text-green-600 dark:text-green-400 hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded cursor-pointer"
                  >
                    <FaArrowUp /> {stock}
                  </li>
                ))}
              ...
              <ul>
                {topLosers.map((stock, idx) => (
                  <li
                    key={idx}
                    className="flex items-center gap-2 text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded cursor-pointer"
                  >
                    <FaArrowDown /> {stock}
                  </li>
                ))}
