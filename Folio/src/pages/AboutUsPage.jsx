      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="mb-10">
          ...
          <div className="space-y-4">
            {[{ question: 'Is FinFolio free to use?', answer: 'Yes! FinFolio is completely free for individual investors and finance learners.' }, { question: 'How real-time is the stock data?', answer: 'We use trusted third-party APIs to fetch near real-time data. Delays may be a few seconds to a minute.' }, { question: 'Do you provide investment advice?', answer: 'No, FinFolio is a tracking and visualization platform only. We do not offer financial advice.' }].map((faq, index) => (
              <div key={index} className="bg-white shadow-md p-4 rounded-lg border border-blue-100 cursor-pointer">
                ...
                <AnimatePresence>
                  {openFAQ === index && (
                    <motion.p initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="text-gray-700 mt-2">
                      {faq.answer}
                    </motion.p>
                  )}
                </AnimatePresence>
        ...
        <motion.section initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }}>
          ...
          <form onSubmit={handleSubmit} className="mb-6">
            <textarea value={review} onChange={(e) => setReview(e.target.value)} placeholder="Share your thoughts about FinFolio..." className="w-full p-4 border rounded-lg resize-none h-28 focus:outline-none focus:ring-2 focus:ring-blue-400"></textarea>
          ...
          {reviews.length > 0 && (
            <div className="space-y-4">
              ...
              {reviews.map((rev, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="bg-white shadow p-4 rounded-lg border border-gray-200 flex items-start space-x-4">
                  <div className="flex-shrink-0 w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold text-xl">
                    {rev.initials || 'U'}
                  </div>
                  <div>
