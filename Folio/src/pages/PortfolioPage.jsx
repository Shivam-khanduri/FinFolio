...
const PortfolioPage = () => {
  ...
  const handleDeletePortfolio = async (symbol) => {
    try {
      await axios.delete(`http://localhost:5000/api/portfolio/${userName}/${symbol}`);
      fetchPortfolio();
      setToast(`🗑️ ${symbol} removed from portfolio`);
      setTimeout(() => setToast(""), 3000);
    } catch (err) {
  ...
  return (
    <div className="flex bg-gray-100 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex-1 p-6 text-gray-800 dark:text-white space-y-10">
        <h2 className="text-3xl font-bold text-blue-600">📊 Your Portfolio</h2>
        <p className="text-lg font-medium text-green-600 mb-2">Total Value: ${totalPortfolioValue}</p>
        {toast && (
          <div className="bg-green-100 text-green-700 p-2 px-4 rounded shadow max-w-xs">
            {toast}
          </div>
            ...
            <tbody>
              {userPortfolio.map((stock, idx) => (
                <tr key={idx} className="border-t dark:border-gray-700">
                  ...
                  <td className="p-3">{(stock.shares * stock.livePrice).toFixed(2)}</td>
                  <td className="p-3">
                    <button onClick={() => handleDeletePortfolio(stock.symbol)} className="text-red-600 hover:underline text-sm">Delete</button>
                  </td>
                </tr>
              ))}
              {userPortfolio.length === 0 && (
                <tr>
                  <td colSpan="6" className="text-center py-4 text-gray-500">No stocks in your portfolio yet.</td>
                </tr>
              )}
            </tbody>
          </table>
